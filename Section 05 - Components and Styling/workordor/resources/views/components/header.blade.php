    ...
    <div class="container mx-auto flex justify-between items-center">
        ...
        <nav class="hidden md:flex items-center space-x-4">
            <a href="{{ url('/jobs') }}"
                ...
                {{ request()->is('jobs') ? 'text-yellow-500 font-bold' : '' }}">All
                Jobs</a>

            <a href="{{ url('/jobs/saved') }}"
                class="text-white hover:underline py-2
                {{ request()->is('jobs/saved') ? 'text-yellow-500 font-bold' : '' }}">Saved
                Jobs</a>

            <a href="{{ url('/login') }}"
                class="text-white hover:underline py-2
                {{ request()->is('login') ? 'text-yellow-500 font-bold' : '' }}">Login</a>

            <a href="{{ url('/register') }}"
                class="text-white hover:underline py-2
                {{ request()->is('register') ? 'text-yellow-500 font-bold' : '' }}">Register</a>

            <a href="{{ url('/dashboard') }}"
                class="text-white hover:underline py-2
                {{ request()->is('dashboard') ? 'text-yellow-500 font-bold' : '' }}">
